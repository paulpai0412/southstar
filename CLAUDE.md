...
`skills/northstar/` is a global Claude skill for *operating* Northstar from a consumer repo (phase commands `/northstar-plan|setup|execute|observe|recover|report`). Its `.mjs` scripts under `scripts/lib/` are covered by `tests/skills/*`. `npm run skill:sync` / `skill:doctor` / `skill:render-config` manage it. This skill is separate from the runtime — changing one rarely requires changing the other.
