...
export async function GET(req: Request) {
  ...
  const cwdRaw = new URL(req.url).searchParams.get("cwd") || process.cwd();
  ...
  try {
    ...
    return Response.json({ error: `Directory does not exist: ${cwdRaw}` }, { status: 400 });
  }
  if (!cwdStat.isDirectory()) {
    return Response.json({ error: `Not a directory: ${cwdRaw}` }, { status: 400 });
  }
