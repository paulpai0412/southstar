...
export function buildLibraryImportCandidatePrompt(
  ...
  const coverageSection = coverageConstraints.length > 0
    ? [
      ...
      "For every artifact/evaluator candidate targeted to a GoalValidationCoverageConstraint, include semanticTags: a non-empty array of short lower-case or kebab-case reusable outcome/domain concepts. These tags are reviewed by the user during import and are later checked against the confirmed Requirement; do not invent host-side vocabulary or copy technical ids.",
      "Target-entry rule: for each blocking constraint, emit one target entry for the selected artifact and a second target entry for the selected evaluator, using the exact same gapRef, requirementId, and complete criterionIds. The evaluator target is mandatory; do not target only the artifact, and do not target an evaluator that does not validate that exact artifact objectKey.",
  ...
  return [
    ...
    "artifact requires artifactType:string, mediaTypes:string[], evidenceKinds:string[], validationRules:string[], schemaRef:string, requiredFields:string[], provenanceRequirements:string[], and semanticTags:string[] when it targets a GoalValidationCoverageConstraint.",
    `evaluator requires validatesArtifactRefs:string[], requiredInputs:string[], evidenceKinds:string[], verificationModes:string[], verificationProcedures:{id:string,procedureVersionRef?:string,checkKind:string,instruction:string,allowedEvidenceKinds:string[],oracleRef?:string,oracleVersionRef?:string,parameterSchema?:Record<string,{type:'string'|'number'|'boolean'|'string[]'|'object',required?:boolean}>}[], independencePolicy:'independent', resultSchemaRef:'${REQUIREMENT_EVALUATOR_RESULT_SCHEMA_REF}', failureClassifications:string[], and semanticTags:string[] when it targets a GoalValidationCoverageConstraint.`,
...
function buildLibraryImportRepairPrompt(basePrompt: string, previousOutput: unknown, issue: string): string {
  ...
  return [
    ...
    "Re-run the complete closure check before responding: every required evidence kind must be present in the artifact, evaluator, and at least one compatible evaluator procedure; the evaluator must validate the exact artifact key; both candidates must have target entries with the same gapRef, requirementId, and complete criterionIds. Do not return a candidate pair that only has matching names or a target entry without compatible contract fields.",
  ].join("\n");
}
