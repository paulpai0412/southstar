  ...
  try {
    const goalContract = finalizeGoalContract({
      ...
      interpretation: {
        ...
        requirements: [
          { statement: "The offline article renders", acceptanceCriteria: [deterministicCriterion("The article opens without network access")], blocking: true, source: "explicit", expectedArtifacts: [] },
          { statement: "A print stylesheet is available", acceptanceCriteria: [{ ...deterministicCriterion("Printing uses a readable layout"), blocking: false }], blocking: false, source: "explicit", expectedArtifacts: [] },
        ],
  ...
  try {
    const goalContract = finalizeGoalContract({
      ...
      interpretation: {
        ...
        requirements: [{ statement: "Mission projections remain bounded", acceptanceCriteria: [deterministicCriterion("Twenty runs do not add per-run queries")], blocking: true, source: "explicit", expectedArtifacts: [] }],
        expectedArtifactRefs: ["artifact.implementation_report", "artifact.verification_report"],
        requiredCapabilities: ["capability.repo-read", "capability.repo-write", "capability.test-execution"],
