  ...
  await assert.rejects(
    () => interpretGoalContractWithLlm({
      ...
      client: { generateText: async () => JSON.stringify({
        ...
        workType: "software_feature",
        summary: "Build it",
        requirements: [{ statement: "Build it", acceptanceCriteria: [], blocking: true, source: "explicit", expectedArtifacts: [] }],
    ...
    client: {
      async generateText(input) {
        ...
        summary: "Deliver a production-ready membership subscription flow",
          requirements: [
            { statement: "Authorized members can access subscription-only features", acceptanceCriteria: [criterion("Unauthorized users are denied and authorized members are allowed")], blocking: true, source: "explicit", expectedArtifacts: [] },
            { statement: "Members can purchase a subscription and payment state is persisted", acceptanceCriteria: [criterion("A successful payment activates exactly one subscription")], blocking: true, source: "explicit", expectedArtifacts: [] },
            { statement: "Members can cancel and receive the configured refund behavior", acceptanceCriteria: [criterion("Cancellation and refund state are observable and idempotent")], blocking: true, source: "explicit", expectedArtifacts: [] },
            { statement: "Operators can inspect subscription and audit events", acceptanceCriteria: [criterion("Administrative reporting shows the recorded lifecycle events")], blocking: true, source: "explicit", expectedArtifacts: [] },
          ],
