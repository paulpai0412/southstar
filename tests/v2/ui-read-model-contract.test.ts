...
test("run-control read model exposes run control contract", async () => {
  ...
  try {
    ...
    assert.ok(model.attentionItems.some((item) => item.id === "exception:runtime-exception:run-ui-control:task-a"));
    assert.ok(model.sourceRefs.some((ref) => ref.ref === `southstar.workflow_runs:${runId}`));
  } finally {
    await db.close();
  }
});
...
test("workflow-dag read model computes dependency readiness", async () => {
  ...
  try {
    ...
    assert.equal(model.data.nodes.find((node) => node.id === "task-c")?.dependencyReady, false);
  } finally {
    await db.close();
  }
});
