...
test("09 regression rollback: monitor rolls back low-risk asset and raises high-risk alert", async () => {
  ...
  try {
    const draft = await createPostgresPlannerDraft(env.db, {
      goalPrompt: "regression rollback real E2E: detect regressions and apply rollback policy",
    });
    const run = await createPostgresRunFromDraft(env.db, { draftId: draft.draftId });

    const promptV1 = await createAssetVersion(env.db, {
    ...
    const promptV2 = await createAssetVersion(env.db, {
      assetKind: "prompt_template",
      assetRef: "prompt-software-maker",
      version: "v2",
      parentVersion: "v1",
    ...
    await promoteAssetVersion(env.db, {
      assetId: promptV2.id,
      actor: "operator",
      reason: "sandbox passed for prompt revision",
      targetStatus: "active",
    });

    const profileV1 = await createAssetVersion(env.db, {
    ...
    const profileV2 = await createAssetVersion(env.db, {
      assetKind: "agent_profile",
      assetRef: "software-maker-pi",
      version: "v2",
      parentVersion: "v1",
    ...
    await promoteAssetVersion(env.db, {
      assetId: profileV2.id,
      actor: "operator",
      reason: "approved high-risk profile rollout",
      targetStatus: "active",
    });

    await recordAssetRegressionObservation(env.db, {
      assetId: promptV2.id,
      riskTier: "low",
    ...
    const monitor = await runRegressionMonitor(env.db, {
      actor: "regression-monitor",
      reason: "scheduled real E2E regression sweep",
    });
    assert.equal(monitor.rollbacks.length, 1);
    assert.equal(monitor.alerts.length, 1);

    const rollback = monitor.rollbacks[0];
    assert.equal(rollback?.rolledBackFromAssetId, promptV2.id);
    assert.equal(rollback?.activeAssetId, promptV1.id);

    const alert = monitor.alerts[0];
    assert.equal(alert?.assetId, profileV2.id);
    ...
    assert.equal(assetRows.rows.some((row) => row.resource_key === promptV1.id && row.status === "active"), true);
    assert.equal(assetRows.rows.some((row) => row.resource_key === promptV2.id && row.status === "rolled_back"), true);
    assert.equal(assetRows.rows.some((row) => row.resource_key === profileV2.id && row.status === "active"), true);
    ...
    assert.equal(observationRows.rows.some((row) => row.payload_json.assetId === promptV2.id && row.status === "rolled_back"), true);
    assert.equal(observationRows.rows.some((row) => row.payload_json.assetId === profileV2.id && row.status === "alerted"), true);
    ...
    assert.equal(rollbackEdges.rows.some((row) => row.edge_type === "ROLLED_BACK_TO" && row.to_node_id === promptV1.id), true);
    assert.equal(rollbackEdges.rows.some((row) => row.edge_type === "HURT" && row.from_node_id === promptV2.id), true);
