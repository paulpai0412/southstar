...
test("Library workspace follows AppShell sidebar plus center chat and file viewer layout", () => {
  ...
  assert.match(workspace, /const canRenderLibraryChat = context\.selectedChatSession \|\| context\.libraryNewSessionCwd/);
  ...
  assert.match(chatWindow, /sessionKind\?: SessionKind/);
  assert.match(chatWindow, /useAgentSession/);
  assert.match(source("web/hooks/useAgentSession.ts"), /sessionKind = opts\.sessionKind/);
